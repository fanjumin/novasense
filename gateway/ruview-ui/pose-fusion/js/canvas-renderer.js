...
export class CanvasRenderer {
  ...
  drawSkeleton(ctx, keypoints, width, height, opts = {}) {
    ...
    if (opts.label) {
      ...
      ctx.globalAlpha = 0.8;
      ctx.fillText(`${opts.label} · ${visCount} ${i18n.t('canvas.joints')}`, 8, height - 8);
      ctx.globalAlpha = 1;
    }
  }
  ...
  drawCsiHeatmap(ctx, heatmap, canvasW, canvasH) {
    ...
    if (!heatmap || !heatmap.data || heatmap.height < 2) {
      ...
      ctx.fillText(i18n.t('canvas.waitingCsi'), 8, canvasH / 2);
      return;
    }
    ...
    ctx.fillText(i18n.t('canvas.subcarrierAxis'), 4, canvasH - 4);
    ctx.save();
    ctx.translate(canvasW - 4, canvasH - 4);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText(i18n.t('canvas.timeAxis'), 0, 0);
    ctx.restore();
  }
  ...
  drawEmbeddingSpace(ctx, points, w, h) {
    ...
    ctx.font = '9px "JetBrains Mono", monospace';
    const legends = [
      { color: this.colors.videoEmb, label: i18n.t('canvas.video') },
      { color: this.colors.csiEmb, label: 'CSI' },
      { color: this.colors.fusedEmb, label: i18n.t('canvas.fused') },
    ];
    legends.forEach((l, i) => {
      const ly = 12 + i * 14;
